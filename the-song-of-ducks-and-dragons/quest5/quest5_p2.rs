// Wyatt Geckle
//
// Everybody Codes 2025 Quest 5 Part 2
    ...
    match args.get(1) {
        Some(file_name) => match File::open(file_name) {
            Ok(file) => {
                ...
                let mut weak_sword = &swords[0];
                let mut strong_sword = &swords[0];

                for sword in swords.iter().skip(1) {
                    if sword.quality < weak_sword.quality {
                        weak_sword = sword;
                    } else if sword.quality > strong_sword.quality {
                        strong_sword = sword;
                    }
                }

                println!("Weakest Sword: ({}, {})", weak_sword.id, weak_sword.quality);
                println!(
                    "Strongest Sword: ({}, {})",
                    strong_sword.id, strong_sword.quality
                );
                println!(
                    "Strength Difference: {}",
                    strong_sword.quality - weak_sword.quality
                );
            }
            Err(e) => {
                eprintln!("{}", e);
